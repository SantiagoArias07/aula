...
// ─────────────────────────────────────────────────────────────────────
// TUTOR IA — chat que explica paso a paso
// ─────────────────────────────────────────────────────────────────────
function respuestaTutor(texto) {
  const t = texto.toLowerCase();
  if (t.includes('fracc') || /\d+\/\d+/.test(t)) return { pasos: [
    "¡Va! Con fracciones el truco casi siempre es el denominador (el de abajo).",
    "Si los denominadores son iguales, sumas o restas solo los de arriba.",
    "Si son distintos, busca un denominador común (el mínimo común múltiplo) y convierte cada fracción.",
    "Ya con el mismo denominador, operas los numeradores y simplificas al final.",
    "Mándame la fracción exacta y la resolvemos juntos, paso por paso. 🙂",
  ] };
  if (t.includes('despej') || t.includes('ecuac') || t.includes('x ') || t.includes(' x') || t === 'x') return { pasos: [
    "Despejar x es dejarla sola de un lado del igual.",
    "Lo que está sumando pasa restando, y lo que está restando pasa sumando.",
    "Lo que está multiplicando pasa dividiendo, y al revés.",
    "Ejemplo: en x + 7 = 12, el 7 está sumando, así que pasa restando: x = 12 − 7 = 5.",
    "¿Me pasas tu ecuación y la despejamos?",
  ] };
  if (t.includes('negativ') || t.includes('entero') || t.includes('signo')) return { pasos: [
    "Imagina una recta numérica: a la derecha crece, a la izquierda baja del cero.",
    "Sumar es moverte a la derecha; restar es moverte a la izquierda.",
    "Dos negativos que se suman se hacen más negativos: −4 + (−6) = −10.",
    "Un negativo más un positivo: te mueves hacia el cero. −5 + 12 = 7.",
    "Dime tu operación y la caminamos en la recta juntos.",
  ] };
  if (t.includes('porcent') || t.includes('%')) return { pasos: [
    "Un porcentaje es 'de cada 100'. El 20% es 20 de cada 100, o sea 20/100 = 0.2.",
    "Para sacar el 20% de un número, multiplícalo por 0.2.",
    "Ejemplo: 20% de 150 = 150 × 0.2 = 30.",
    "¿De qué número quieres sacar el porcentaje?",
  ] };
  return { txt: "¡Con gusto te ayudo! Escríbeme el problema tal cual, o tócale a 📷 Escanear para tomarle foto. Dime también de qué tema es (fracciones, ecuaciones, porcentajes…) y lo resolvemos paso a paso, sin que te lo dé mascado. 💪" };
}
...
function VistaTutor({ tw }) {
  ...
  const enviar = (textoDirecto) => {
    ...
    setEnviando(true);
    setTimeout(() => {
      setMensajes(prev => [...prev, { mio: false, hora: 'ahora', ...respuestaTutor(txt) }]);
      setEnviando(false);
    }, 1400);
  };
  ...
  const escanear = () => {
    ...
    setEnviando(true);
    setTimeout(() => {
      setMensajes(prev => [...prev, { mio: false, hora: 'ahora', pasos: [
        "¡Leí tu foto! Es 1/2 + 1/3. Vamos paso a paso:",
        "Denominadores distintos (2 y 3). El común más pequeño es 6.",
        "Convierte: 1/2 = 3/6 y 1/3 = 2/6.",
        "Suma los de arriba: 3 + 2 = 5.",
        "Resultado: 5/6. ¿Quieres uno parecido para practicar tú?",
      ] }]);
      setEnviando(false);
    }, 1600);
  };
  ...
  return (
    <div className="view">
      <SectionTitle eyebrow="Tutor de matemáticas · IA"
        title={<>Pregúntame lo que <span className="serif-italic">sea</span>.</>}
        subtitle="Te explico paso a paso, a tu ritmo. Nunca te doy la respuesta pelona: la idea es que entiendas."
      />
      ...
      <div className="card card-flush h-auto-sm" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 300px)', minHeight: 500, overflow: 'hidden', maxWidth: 820 }}>
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--primary-soft)', color: 'var(--primary-ink)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <IconBrain size={20} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14.5, fontWeight: 600 }}>Tutor Aula</div>
            ...
            <div key={i} style={{ display: 'flex', justifyContent: b.mio ? 'flex-end' : 'flex-start' }}>
              <div style={{ maxWidth: '80%' }}>
                <div style={{
                  ...
                  padding: b.pasos ? '14px 16px' : '12px 16px',
                  borderRadius: b.mio ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                      {b.pasos.map((p, j) => (
                        <div key={j} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                          {j === 0 || j === b.pasos.length - 1 ? (
                            <span style={{ marginTop: 1 }}><Expr>{p}</Expr></span>
                          ) : (
                            <>
        ...
        <div style={{ borderTop: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', gap: 8, padding: '12px 14px 0', overflowX: 'auto' }}>
            {TUTOR_SUGERENCIAS.map((sg, i) => (
              <button key={i} className="suger" onClick={() => enviar(sg)}>{sg}</button>
            ))}
          </div>
          <div style={{ padding: 14, display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            ...
            <textarea value={draft} onChange={(e) => setDraft(e.target.value)}
              ...
              placeholder="Escribe tu duda o un problema…" rows={1}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="card">
            ...
            <p style={{ fontSize: 13.5, color: 'var(--ink-2)', marginTop: 10 }}>
              Recibe un resumen semanal del avance de tu hijo: qué domina, en qué va atrasado y cuánto practicó. En lenguaje claro, sin tecnicismos.
            </p>
          ...
          <div className="card">
            ...
            <p style={{ fontSize: 13.5, color: 'var(--ink-2)', marginTop: 10 }}>
              Descarga tu ruta con señal y practica offline. Tu avance se sincroniza solo al reconectar.
            </p>
...
function VistaCuenta() {
  ...
  const [etapaSel, setEtapaSel] = v3State(ESTUDIANTE.etapa);
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '1px solid var(--line)', overflowX: 'auto' }}>
        {[['perfil', 'Perfil'], ['nivel', 'Nivel y ruta'], ['pref', 'Preferencias'], ['priv', 'Privacidad']].map(([id, label]) => (
          <a key={id} onClick={() => setTab(id)} style={{
            padding: '10px 16px', fontSize: 14, fontWeight: 500, whiteSpace: 'nowrap',
            color: tab === id ? 'var(--ink)' : 'var(--muted)',
            borderBottom: tab === id ? '2px solid var(--ink)' : '2px solid transparent', marginBottom: -1,
          }}>{label}</a>
        ))}
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="card card-lg">
              ...
              <div className="grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
                ...
                <CuStat label="Skills" value={ESTADISTICAS.skillsDominadas} />
              ...
              <div style={{ marginTop: 18 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--muted)', marginTop: 8 }}>
                  <span>Dominio de {ESTUDIANTE.nivelLabel}</span>
                  <span>meta: 100%</span>
                </div>
              </div>
            </div>
      ...
      {tab === 'nivel' && (
        <div style={{ maxWidth: 760 }}>
          <div className="card card-lg">
            ...
            <p style={{ fontSize: 14, color: 'var(--muted)', marginTop: 6 }}>Aula funciona desde preescolar hasta preparatoria. Elige tu etapa y regeneramos tu ruta con los aprendizajes clave de ese nivel.</p>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 18 }}>
              {ETAPAS.map(e => (
                <button key={e.id} className="chip-etapa" data-on={etapaSel === e.nombre ? '1' : '0'} onClick={() => setEtapaSel(e.nombre)}>
                  {e.nombre} <span className="mono" style={{ opacity: .6, fontSize: 11 }}>· {e.grados}</span>
                </button>
              ))}
            </div>
            {etapaSel !== ESTUDIANTE.etapa && (
              <div style={{ marginTop: 16, padding: '12px 14px', background: 'var(--primary-soft)', borderRadius: 12, fontSize: 13.5, color: 'var(--ink-2)', display: 'flex', gap: 10 }}>
                <IconRoute size={17} style={{ color: 'var(--primary-ink)', flexShrink: 0 }} />
                Al cambiar a <strong>{etapaSel}</strong>, tu ruta se regenerará con un nuevo diagnóstico para ese nivel.
              </div>
            )}
          </div>
            ...
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column' }}>
              {ETAPAS.map((e, i) => (
                <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderTop: i ? '1px solid var(--line)' : 'none' }}>
                  <Glyph color={e.color} size={38} radius={11}>
                    {e.id === 'prees' ? <IconShapes size={17} /> : e.id === 'prim' ? <IconDivide size={17} /> : e.id === 'sec' ? <IconFx size={17} /> : <IconChart size={17} />}
                  </Glyph>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 500 }}>{e.nombre} <span className="mono" style={{ fontSize: 11, color: 'var(--muted)' }}>· {e.grados}</span></div>
                    <div style={{ fontSize: 12.5, color: 'var(--muted)', marginTop: 2 }}>{e.desc}</div>
                  </div>
                  {e.nombre === ESTUDIANTE.etapa && <span className="pill pill-primary" style={{ fontSize: 10 }}>tu nivel</span>}
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === 'pref' && (
        <div className="card card-lg" style={{ maxWidth: 720 }}>
          ...
          {[
            ...
            ['Datos móviles', 'Descargar prácticas solo en Wi-Fi'],
          ].map(([k, v], i) => (
        ...
        <div className="card card-lg" style={{ maxWidth: 720 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              ...
              ['Compartir avance con mi familia', 'Resumen semanal a mamá', 'Ajustar'],
...
function CuStat({ label, value }) {
  return (
    <div>
      ...
      <div style={{ fontFamily: 'var(--font-serif)', fontSize: 26, marginTop: 4 }}>{value}</div>
    </div>
  );
}
